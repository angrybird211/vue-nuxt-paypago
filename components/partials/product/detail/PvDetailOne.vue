	...
	<div>
		...
		>
			<template v-if="!product.is_sale">
				<span class="new-price">R{{ product.price | priceFormat }}</span>
			</template>

			<template v-else>
				<span class="new-price">R{{ product.sale_price | priceFormat }}</span>
				<span class="old-price">R{{ product.price | priceFormat }}</span>
			</template>
		</div>
		...
		>
			<template v-if="minPrice !== maxPrice">
				<span class="new-price">R{{ minPrice | priceFormat }} &ndash; ${{ maxPrice | priceFormat }}</span>
			</template>

			<template v-else>
				<span class="new-price">R{{ minPrice | priceFormat }}</span>
			</template>
		</div>
		...
		>
			<div class="product-single-filter select-custom">
				...
				>
					...
					<option
						:value="disColors[index]"
						v-for="(item, index) in disColors"
						:key="'color' + index"
					>{{disColors[index].name.toUpperCase()}}</option>
				</select>
			</div>
			...
			<div class="product-single-filter select-custom">
				...
				>
					...
					>CHOOSE AN OPTION</option>
					<option
						:value="disSizes[index]"
						v-for="(item, index) in disSizes"
						:key="'size' + index"
					>{{disSizes[index].name.toUpperCase()}}</option>
				</select>
			</div>
			<!-- <div
			...
			>
				...
				>
					<template v-if="product.variants[curIndex].sale_price">
						<del class="old-price">
							<span>R{{ product.variants[curIndex].price | priceFormat }}</span>
						</del>
						<span class="product-price">R{{ product.variants[curIndex].sale_price | priceFormat }}</span>
					</template>

					<template v-else>
						<span class="product-price">R{{ product.variants[curIndex].price | priceFormat }}</span>
					</template>
				</div>
			</vue-slide-toggle>
...
export default {
	...
	methods: {
		...
		colorHandler: function() {
			this.disSizes = [];
			for (let i = 0; i < this.vSizes.length; i++) {
				if (!this.isDisabled(this.curColor, this.vSizes[i]))
					this.disSizes.push(this.vSizes[i]);
			}
		},
		sizeHandler: function() {
			this.disColors = [];
			for (let i = 0; i < this.vColors.length; i++) {
				if (!this.isDisabled(this.vColors[i], this.curSize))
					this.disColors.push(this.vColors[i]);
			}
		}
	}
};
